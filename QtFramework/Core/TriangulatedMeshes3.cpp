...
GLvoid TriangulatedMesh3::DeleteVertexBufferObjects()
{
    ...
    // homework: delete vertex buffer objects of unit normal vectors, texture coordinates, and indices ._.
    if (_vbo_normals)
    {
...
std::ostream& cagd::operator <<(std::ostream& lhs, const TriangulatedMesh3& rhs)
{
    ...
    lhs << rhs._leftmost_vertex << endl;
    lhs << rhs._rightmost_vertex << endl;
    ...
    for (std::vector<DCoordinate3>::const_iterator nit = rhs._normal.begin(); nit != rhs._normal.end(); ++nit)
    {
        lhs << *nit << std::endl;
    }

    for (std::vector<TCoordinate4>::const_iterator texit = rhs._tex.begin(); texit != rhs._tex.end(); ++texit)
    {
        lhs << *texit << std::endl;
    }

    for (std::vector<TriangularFace>::const_iterator fit = rhs._face.begin(); fit != rhs._face.end(); ++fit)
    {
        lhs << *fit << std::endl;
    }
