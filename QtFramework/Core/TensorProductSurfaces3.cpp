    ...
    for (GLuint i = 0; i < u_div_point_count; ++i)
    {
        ...
        for (GLuint j = 0; j < v_div_point_count; ++j)
        {
            ...
            */
            ...
            {
                cout << "baj van calpartderivnel" << endl;
            }
...
TensorProductSurface3::TensorProductSurface3(const TensorProductSurface3& surface): _u_min(surface._u_min),
...
GLboolean TensorProductSurface3::UpdateVertexBufferObjectsOfData(GLenum usage_flag)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, _vbo_data);
    glBufferData(GL_ARRAY_BUFFER, 2 * row_count * column_count * 3 * sizeof(GLfloat), 0, usage_flag);
...
{
    RowMatrix<GenericCurve3*> *isoParametricLine = new RowMatrix<GenericCurve3*> (iso_line_count);

    GLdouble u_lepes = (_u_max - _u_min) / (div_point_count - 1);
    GLdouble v_lepes = (_v_max - _v_min) / (iso_line_count - 1);

    for(GLuint i = 0; i < iso_line_count; i++)
    {
        (*isoParametricLine)[i] = new GenericCurve3(maximum_order_of_derivatives, div_point_count, usage_flag);
        GLdouble v = min(_v_min + v_lepes * i, _v_max);

        for (GLuint j = 0; j < div_point_count; j++)
        {
            GLdouble u = min(_u_min + u_lepes * j, _u_max);
            PartialDerivatives pd;

            if (!CalculatePartialDerivatives(maximum_order_of_derivatives, u, v, pd))
            {
                for (GLuint ii = 0; ii <= i; ii++)
                {
                    delete (*isoParametricLine)[ii];
                    (*isoParametricLine)[ii] = nullptr;
                }

                delete isoParametricLine;
                isoParametricLine = nullptr;

                return isoParametricLine;
            }

            for (GLuint r = 0; r <= maximum_order_of_derivatives; r++)
            {
                (*(*isoParametricLine)[i])(r, j) = pd(r, 0);
            }
        }
    }
                                                      ...
                                                      GLenum usage_flag) const
{
    ...
    GLdouble u_lepes = (_u_max - _u_min) / (iso_line_count - 1);
    GLdouble v_lepes = (_v_max - _v_min) / (div_point_count - 1);

    for(GLuint i = 0; i < iso_line_count; i++)
    {
        (*isoParametricLine)[i] = new GenericCurve3(maximum_order_of_derivatives, div_point_count, usage_flag);
        GLdouble u = min(_u_min + u_lepes * i, _u_max);

        for (GLuint j = 0; j < div_point_count; j++)
        {
            GLdouble v = min(_v_min + v_lepes * j, _v_max);
            PartialDerivatives pd;
            ...
            for (GLuint r = 0; r <= maximum_order_of_derivatives; r++)
            {
                (*(*isoParametricLine)[i])(r, j) = pd(r, r);
            }
        }
    }
